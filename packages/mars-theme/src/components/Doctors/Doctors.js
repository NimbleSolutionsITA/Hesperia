    ...
    useEffect(() => {
        async function fetchDoctors() {
            const response = await libraries.source.api.get({
                endpoint: "doctors",
                params: {per_page: serviceId ? 30 : 100, _embed: true,},
            });
            const res = await libraries.source.populate({ response, state })
            return res.map(({id}) => state.source.doctors[id])
        }
                                        ...
                                        <Card elevation={0}>
                                            <CardActionArea>
                                                <CardMedia
                                                    ...
                                                    image={state.source.attachment[doctor["featured_media"]]['media_details']['sizes']['thumbnail']['source_url']}
                                                    style={{height: '300px'}}
                                                />
                                                <CardContent>
